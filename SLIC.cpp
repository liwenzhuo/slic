...
{
// 	RSImage img;
// 	CString str("C:\\quick_bird_SLIC.tif");
// 	CString strfmt("GTIFF");
// 	img.New(str,strfmt,1,height,width,RSS_INT32);
// 	HRSBAND hband = img.GetBand(0);
// 	img.WriteBlock(hband,0,0,width,height,(void*)labels);
// 	img.Destroy();
	...
	outfile.write((const char*)&width, sizeof(int));
	outfile.write((const char*)&height, sizeof(int));
	for( int i = 0; i < sz; i++ )
	{
	...
	const double&				m)//weight given to spatial distance
{
	...
	PerformSuperpixelSegmentation_VariableSandM(kseedsl,kseedsa,kseedsb,kseedsx,kseedsy,klabels,STEP,15);
	numlabels = kseedsl.size();
